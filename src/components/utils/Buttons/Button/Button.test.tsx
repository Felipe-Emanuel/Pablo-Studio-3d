...
describe("<Button />", () => {
  ...
  it("should render correctly 'light' variant and size 'lg'", () => {
    ...
    expect(getByRole('button')).toHaveClass('bg-transparent border-2 border-white text-lg')
  });

  it("should render correctly 'dark' variant and size 'sm", () => {
    const { getByRole } = render(<Button labelSize="sm" label="Acessar" variant="dark" />)

    expect(getByRole('button')).toHaveClass('bg-dark rounded-md shadow-button text-sm')
  })
